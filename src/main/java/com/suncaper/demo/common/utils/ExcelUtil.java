...
public class ExcelUtil {
    ...
    public static void exportApplication(HttpServletResponse response, List<Application> list) throws IOException {
        //声明一个工作簿
        HSSFWorkbook hwb = new HSSFWorkbook();
        //声明一个sheet并命名
        HSSFSheet sheet = hwb.createSheet("寒衣申请表");
        //给sheet名称一个长度
        sheet.setDefaultColumnWidth((short) 10);

        sheet.setColumnWidth(0, "疑似-双向情感障碍 ".getBytes().length * 256);
        sheet.setColumnWidth(1, "数学与计算机学院".getBytes().length * 256);
        sheet.setColumnWidth(2, "12345678999  ".getBytes().length * 256);
        sheet.setColumnWidth(3, "疑似-双向情感障碍疑似-双向情感障碍疑似-双向情感障碍  ".getBytes().length * 256);
        ...
        for (short i = 0; i < list.size(); i++) {
            ...
            cell.setCellValue(list.get(i).getBatch().getName());
            ...
            cell.setCellStyle(commStyle);
            cell.setCellValue(list.get(i).getDifValue());

            cell = row.createCell(3);
            cell.setCellStyle(commStyle);
            cell.setCellValue(list.get(i).getApplicationReason());

            cell = row.createCell(4);
            cell.setCellStyle(commStyle);
            cell.setCellValue(list.get(i).getAuditComment());

            cell = row.createCell(5);
            cell.setCellStyle(commStyle);
            cell.setCellValue(list.get(i).getStatusValue());
        }
